...
func TestInfoDepth(t *testing.T) {
	...
	f := func() { InfoDepth(context.Background(), 1, "depth-test1") }

	// The next three lines must stay together
	_, _, wantLine, _ := runtime.Caller(0)
	InfoDepth(context.Background(), 0, "depth-test0")
	f()
	...
	for i, m := range msgs {
		if !strings.HasPrefix(m, string(Green)+string(Bold)+"I") {
		...
		// trim whitespace
		msg = strings.TrimRight(msg[:x], " ")
		line, err := strconv.Atoi(msg)
		if err != nil {
...
func TestV(t *testing.T) {
	...
	V(2).Info(context.Background(), "test")
	if !contains(infoLog, "I", t) {
		t.Errorf("Info has wrong character: %q", contents(infoLog))
	}
	if !contains(infoLog, "test", t) {
